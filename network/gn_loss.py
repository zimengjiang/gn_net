...
class GNLoss(nn.Module):
    ...
    def forward(self, F_a, F_b, known_matches):
        ...
        fa_4_sliced = self.extract_features(F_a[-1], known_matches['a']/4) # //4 if the input image is scaled
        # fa_4_sliced_reshape = fa_4_sliced.reshape((B,N,C))
        fb_4_sliced = self.extract_features(F_b[-1], known_matches['b']/4) # //4
        ...
        for i in range(len(F_a)):
            ...
            if (i == 3):
                ...
                fa_sliced_pos = self.extract_features(F_a[i], known_matches['a'] / (level*4)) #(level*4)) #TODO: use bilinear interpolation in extract_features
                fb_sliced_pos = self.extract_features(F_b[i], known_matches['b'] / (level*4)) #(level*4))
            ...
            '''compute gn loss'''
            loss_gn = self.compute_gn_loss(fa_sliced_pos, F_b[i], known_matches['b'] / (level*4), (level)) #//4, //no 4 for the second level
